...
|---|---|---|---|
| `assets-path` | Path to the assets directory to scan | No | `assets` |
| `working-directory` | Directory containing `pubspec.yaml` | No | `.` |
| `blocking` | Fail the job when unused assets are found | No | `false` |

#### Outputs

| Name | Description |
|---|---|
| `unused-count` | Number of unused assets detected |

By default (`blocking: "false"`) the action reports and deletes unused assets but never fails the build — set `blocking: "true"` to turn it into a hard PR check instead.
